...
import { getTopTracks, searchTrack } from '../services/last-fm';
import { BsSearch, BsFillMusicPlayerFill, BsChevronCompactLeft, BsChevronCompactRight } from "react-icons/bs";
...
function Tracks() {
  ...
  async function getTracks() {
    setLoading(true)
    await getTopTracks(currentPage.current).then(res => {
  ...
  async function search() {
    ...
    if (value) {
      setSearchMode(true)
      setLoading(true)
      const result = await searchTrack(value, currentPage.current)
      setTrackList(result.results.trackmatches)
      setLoading(false)
    } else {
      setSearchMode(false)
          ...
          return (
            <a href={item.url} target="_blank" key={i} className=' z-20 w-full md:w-52 h-52 shrink-0 bg-transparent backdrop-blur-2xl shadow-lg rounded-xl outline outline-1 outline-offset-0 outline-gray-200 flex flex-col justify-start items-stretch p-4'>
              ...
              <div className='text-white flex flex-row items-center justify-start gap-2'>
                ...
                {numeral(searchMode ? item.listeners : item.playcount).format('0,0')}
              </div>
            </a>
          )
        })}
